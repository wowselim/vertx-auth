...
public class WebAuthnImpl implements WebAuthn {
  ...
  public void authenticate(JsonObject authInfo, Handler<AsyncResult<User>> handler) {
    try {
      ...
      //      "type": "public-key"
      //    }
      final JsonObject webauthnResp = getNonNullObject(authInfo, "webauthn");
      final JsonObject response = getNonNullObject(webauthnResp, "response");

      byte[] clientDataJSON = b64dec.decode(getNonNull(response, "clientDataJSON"));
      JsonObject clientData = new JsonObject(Buffer.buffer(clientDataJSON));

      // Verify challenge is match with session
      if (!clientData.getString("challenge").equals(getNonEmpty(authInfo, "challenge"))) {
      ...
      switch (clientData.getString("type")) {
        case "webauthn.create":
          try {
            final String username = getNonEmpty(authInfo, "username");
